...
public interface NotificationRepository extends JpaRepository<Notification, UUID> {
    ...
    @Modifying
    @Query("UPDATE Notification n SET n.deliveryStatus = :status, n.updatedAt = :now WHERE n.id = :id")
    int updateDeliveryStatus(@Param("id") UUID id, @Param("status") NotificationDeliveryStatus status, @Param("now") Instant now);
}
