...
class DonationEventController extends ChangeNotifier {
  ...
  Future<void> loadEvents() async {
    ...
    try {
      ...
      _errorMessage = e is NetworkException
          ? e.message
          : 'Unable to load donation camps. Please check your connection.';
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  ...
  Future<void> loadNearbyEvents(
    ...
    try {
      ...
      _errorMessage =
          e is NetworkException ? e.message : 'Unable to find nearby camps.';
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  ...
  Future<void> loadEventDetails(String eventId) async {
    ...
    try {
      ...
      _errorMessage =
          e is NetworkException ? e.message : 'Failed to load camp details.';
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  ...
  Future<bool> registerForEvent(String eventId) async {
    ...
    try {
      ...
      if (e is ConflictException) {
        ...
        } else if (e.message.contains('EVENT_FULL') ||
            e.message.toLowerCase().contains('capacity')) {
          _errorMessage =
              'This donation camp has reached maximum capacity. Please check back later or view nearby camps.';
        } else {
  ...
  Future<bool> cancelRegistration(String eventId) async {
    ...
    try {
      ...
      _currentRegistration = reg;
      _actionSuccessMessage =
          'Your registration for this donation camp has been cancelled.';
      if (_currentEvent != null && _currentEvent!.id == eventId) {
        _currentEvent = await _apiService.getEventDetails(eventId);
      }
      return true;
      ...
      _errorMessage =
          e is NetworkException ? e.message : 'Could not cancel registration.';
      return false;
    } finally {
      _isActionLoading = false;
      notifyListeners();
  ...
  Future<void> loadMyRegistrations() async {
    ...
    try {
      ...
      _errorMessage =
          e is NetworkException ? e.message : 'Unable to load registrations.';
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  ...
  Future<DonationEventDetail?> createEvent(Map<String, dynamic> payload) async {
    ...
    try {
      ...
      _errorMessage =
          e is NetworkException ? e.message : 'Failed to create donation camp.';
      return null;
    } finally {
      _isActionLoading = false;
      notifyListeners();
  ...
  Future<DonationEventDetail?> submitEvent(String eventId) async {
    ...
    try {
      ...
      _errorMessage =
          e is NetworkException ? e.message : 'Failed to submit donation camp.';
      return null;
    } finally {
      _isActionLoading = false;
      notifyListeners();
